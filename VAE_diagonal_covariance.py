...
Encoder is a Gaussian with diagonal covariance matrix and decoder is a Gaussian with diagonal covariance matrix
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.fc22 = nn.Linear(h_dim, image_size)   # log_var
    ...
    def forward(self, z):
        ...
        mu_decoder = self.sigmoid(self.fc21(h1))
        log_var_decoder = self.f(self.fc22(h1))
        return mu_decoder, log_var_decoder   # mean and covariance of decoder
...
for epoch in range(1, args.epochs + 1):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        for i in range(sample_num):
            ...
            mu_decoder, log_var_decoder = decoder(z)  # var shape is batch * image_z

            reconstruction_loss += 0.5 * torch.sum(log_var_decoder + (data-mu_decoder)*torch.div(1, log_var_decoder.exp())*(data-mu_decoder))
    ...
    with torch.no_grad():
        for (data1, _) in test_loader:
            ...
            for i in range(sample_num):
                z = reparametrize(mu_encoder, log_var_encoder)
                mu_decoder, log_var_decoder = decoder(z)
                a = torch.div(1, log_var_decoder.exp())
                reconstruction_loss += 0.5 * torch.sum(log_var_decoder + (data1 - mu_decoder) * a * (data1 - mu_decoder))
...
with torch.no_grad():
    for batch_idx1, (data1, _) in enumerate(test_loader):
        ...
        z = reparametrize(mu_encoder, log_var_encoder)
        mu_decoder, log_var_decoder = decoder(z)
        comparison = torch.cat([data1.view(-1, 1, 28, 28),
                                    mu_decoder.view(-1, 1, 28, 28)], dim=3)
        save_image(comparison.cpu(), 'result+recons' + str(batch_idx1) + '.png')

# ------- generate new pictures -----------
z = torch.randn(8, z_dim)
decoder = decoder.eval()
with torch.no_grad():
    mu_new, log_var_new = decoder(z)
